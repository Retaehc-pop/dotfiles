...
const [city, setCity] = createState<string | null>(null);
const [country, setCountry] = createState<string | null>(null);
...
function startWeatherInterval() {
    if (latitude === null || longitude === null) {
        console.error("Cannot start update interval: no lat/lon");
        return;
    }

    // 2️⃣ Watch network connectivity (AGNOSTIC & synchronous)
    ...
    // 3️⃣ Initial run if already online
    if (network.connectivity === AstalNetwork.Connectivity.FULL) {
        updateInterval = interval(WEATHER_UPDATE_INTERVAL, () => {
            updateWeather(); 
        });
...
export default function() {
    setupUpdateInterval()
    onCleanup(() => {
      if (updateInterval) updateInterval.cancel();
    });
  ...
  const locationLabel = createComputed(
    [city,country],
    (c,co) =>{
      return c && co ? ` ${c}, ${co}` :  "Loading..."
    })
    return <box
